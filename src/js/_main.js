      ...
      init: function () {
        ...
        $('a[href^="#"]').not('a[href="#"]').click(function () {
          ...
          $('html, body').delay().animate({
            scrollTop: $(target).offset().top - headerHeight - convertedREM
          }, 1000);
        ...
        $("a").not('a[href^="#"], a[href^="mailto"], a[href^="tel:"], .tab-links a, a[download], a[href="#"], .nturl, .modal-toggle, .modal-toggle a, .pop-link, ul.tabs li a, .woocommerce-product-gallery__image a, a[target="_blank"], a[target="blank"], .survey-toggle, .stickylist-fileupload a, #menu-full-mobile li.menu-item-has-children a, div[id^="sticky-list-wrapper"] ul.pagination li a, .pop, .question').click(function (e) {
          ...
          if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
            ...
            e.preventDefault();
            var base = link.split(window.location.protocol + '//' + window.location.host);
            ...
            var style = window.getComputedStyle(el, null).getPropertyValue('font-size');
            var fontSize = parseFloat(style);
            var convertedREM = fontSize * 5;

            console.log(base[0]);
            console.log(base[1]);

            var hash = base[1].split('#');

            console.log(current);

            if (link.includes('#') && (current.includes(hash[0]))) {

              console.log(current.includes('' + base[0] + ''));

              var target = base[1].split('#');

              console.log(target);

              $('html, body').delay().animate({
                scrollTop: $('#' + target[1]).offset().top - headerHeight - convertedREM
              }, 1000);
      ...
      finalize: function () {
        ...
        $('.tab-links a').click(function () {
          if ($(this).is('a[href^="#"]')) {
            ...
            if ($(this).parents('.col-lg-3.col-md-12.col-11.mx-auto').hasClass('visible-xs')) {

              console.log('clicked bottom link');
              document.documentElement.scrollTop = ($('.tab-links').not('.visible-xs .tab-links').offset().top - $('.main-header').outerHeight())
          ...
          $('.stickylist-fileupload').each(function () {
            ...
            if ($(window).width() > 991) {
              ...
              PDFJS.getDocument(image).promise.then(function (doc) {
                var pages = [];
                while (pages.length < 1) pages.push(1);
    ...
    'page_id_14744': {
      init: function () {
        ...
        $(document).ready(function(){
          ...
          window.getSelection().selectAllChildren(document.querySelector('.home-hero h2.main'));
      ...
      init: function () {
        ...
        var insertForm247808 = function () {
          ...
          var html247808 = '<div id="donation-form-container">' +
            ...
            '      <div class="field text CustomTransactionField_563200" >' +
            '<label for="CustomTransactionField_563200"><span class="label">In Tribute To or in Honor Of</span></label>' +
            '<input id="CustomTransactionField_563200" name="CustomTransactionField_563200" type="text" maxlength="255"></input>' +
            '</div>' +
            '' +
            '</div>' +
